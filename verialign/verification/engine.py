...
from verialign.verification.models import VerificationResult, VerifiedClaim, Contradiction, ChecklistItem
...
class VerificationEngine:
    def __init__(self, llm_client: Callable | None = None, web_api_key: str | None = None, web_provider: str = "tavily", cache_ttl: int = 300) -> None:
        self.claim_extractor = ClaimExtractor(llm_client=llm_client)
        self.source_grounder = SourceGrounder(web_api_key=web_api_key, web_provider=web_provider)
        self.contradiction_detector = ContradictionDetector()
        self.confidence_scorer = ConfidenceScorer()
        self.checklist_generator = ChecklistGenerator()
        self._cache = VerificationCache(ttl_seconds=cache_ttl)

    async def verify(self, text: str, context: object, response_data: dict | None = None) -> VerificationResult:
        ...
        for idx, claim_text in enumerate(claim_texts):
            status, confidence, sources = await self.source_grounder.ground(claim_text, context)

            if response_data:
                logprobs_info = self.confidence_scorer.score_response(response_data)
                if logprobs_info:
                    token_logprobs = [logprobs_info["avg_logprob"]] * len(claim_text.split())
                    confidence_score = self.confidence_scorer.score_claim(claim_text, confidence, token_logprobs)
        ...
        checklist = self.checklist_generator.generate(text, claim_texts, [c.to_dict() for c in claims])
