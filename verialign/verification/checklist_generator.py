...
class ChecklistGenerator:
    ACTION_KEYWORDS = {
        "security": ["password", "encrypt", "authentication", "authorization", "token", "secret", "key", "ssl", "tls", "certificate"],
        "deployment": ["deploy", "install", "configure", "setup", "docker", "kubernetes", "k8s", "ci/cd", "pipeline", "environment"],
        "database": ["migration", "schema", "query", "index", "backup", "transaction", "connection", "pool"],
        "api": ["endpoint", "request", "response", "rate limit", "timeout", "retry", "pagination", "version"],
        "testing": ["test", "mock", "assert", "coverage", "integration", "unit", "e2e", "fixture"],
        "performance": ["cache", "optimize", "latency", "throughput", "memory", "cpu", "bottleneck", "profile"],
        "data": ["validate", "transform", "schema", "etl", "pipeline", "quality", "privacy", "pii", "gdpr"],
    }

    CLAIM_CATEGORIES = {
        "factual": ["is", "are", "was", "were", "has", "have", "contains", "supports", "uses", "requires", "returns", "stores"],
        "conditional": ["if", "when", "unless", "provided that", "assuming"],
        "causal": ["because", "since", "therefore", "thus", "leads to", "results in", "causes"],
        "comparative": ["better", "worse", "faster", "slower", "more", "less", "than", "compared to"],
        "temporal": ["before", "after", "during", "while", "since", "until", "by the time"],
    }

    def generate(self, response_text: str, claims: list[str], verification_results: list[dict]) -> list[ModelChecklistItem]:
    ...
    def _generate_verification_items(self, claims: list[str], verification_results: list[dict]) -> list[ModelChecklistItem]:
        items: list[ModelChecklistItem] = []
        ...
        for i, (claim, result) in enumerate(zip(claims, verification_results)):
            ...
            if status == "unsupported":
                items.append(ModelChecklistItem(
    ...
    def _generate_claim_category_items(self, claims: list[str]) -> list[ModelChecklistItem]:
        items: list[ModelChecklistItem] = []
