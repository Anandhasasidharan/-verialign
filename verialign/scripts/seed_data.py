...
CONTEXTS = [
    {"id": "doc-1", "text": "VeriAlign is a verification support proxy for LLM outputs. It intercepts requests and adds verification metadata."},
    {"id": "doc-2", "text": "The Earth orbits the Sun at an average distance of 149.6 million kilometers."},
    {"id": "doc-3", "text": "Python is a high-level programming language created by Guido van Rossum in 1991."},
    {"id": "doc-4", "text": "The capital of France is Paris. The population is approximately 2.1 million."},
    {"id": "doc-5", "text": "Machine learning models require training data. More data generally improves accuracy."},
]

PROMPTS = [
    ("What is VeriAlign?", "VeriAlign is a verification support proxy for LLM outputs. It can extract factual claims and compare them with supplied context."),
    ("How far is Earth from the Sun?", "The Earth orbits the Sun at an average distance of 149.6 million kilometers."),
    ("Tell me about Python", "Python is a high-level programming language created by Guido van Rossum in 1991. It is widely used for web development and data science."),
    ("What is the capital of France?", "The capital of France is Paris. The population is approximately 2.1 million people."),
    ("How does machine learning work?", "Machine learning models require training data. More data generally improves accuracy. The model learns patterns from the data."),
    ...
    ("Explain photosynthesis", "Photosynthesis is the process by which plants convert sunlight into energy. Chlorophyll in leaves captures light energy."),
...
HALLUCINATED_PROMPTS = [
    ("What is VeriAlign?", "VeriAlign is a database management system created by Microsoft in 2020."),
    ("How far is Earth from the Sun?", "The Earth orbits the Sun at an average distance of 50 million kilometers."),
    ("Tell me about Python", "Python is a low-level programming language created by Bill Gates in 1995."),
    ("What is the capital of France?", "The capital of France is Lyon. The population is approximately 500,000."),
    ("How does machine learning work?", "Machine learning models do not require any training data. They work purely by magic."),
]

CONTRADICTION_PROMPTS = [
    ("Compare options", "Option A is better than Option B. Option B is better than Option A."),
    ("Analyze trends", "The stock market is rising. The stock market is falling."),
    ("Evaluate features", "Feature X increases performance. Feature X decreases performance."),
]

TASK_CATEGORIES = ["question_answering", "coding", "writing", "analysis", "summarization", "general"]
...
def generate_verification_data(prompt: str, response: str, context: list) -> dict:
    ...
    for idx, claim_text in enumerate(claim_texts):
        ...
        claims.append({
            ...
            "sources": [{"source_id": s.source_id, "score": s.score, "excerpt": s.excerpt} for s in sources],
...
def create_sample_traces(count: int = 100):
    ...
    for i in range(count):
        ...
        created_at = datetime.now(timezone.utc) - timedelta(days=days_ago, hours=hours_ago, minutes=minutes_ago)
