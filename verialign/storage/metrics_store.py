...
class MetricsStore:
    ...
    def get_overview(self, days: int = 7) -> dict[str, Any]:
        ...
        total_requests = self.session.query(func.count(Trace.id)).filter(Trace.created_at >= cutoff).scalar() or 0

        total_claims = self.session.query(func.count(Claim.id)).join(Trace).filter(Trace.created_at >= cutoff).scalar() or 0

        status_counts = defaultdict(int)
        for status, count in self.session.query(Claim.status, func.count(Claim.id)).join(Trace).filter(Trace.created_at >= cutoff).group_by(Claim.status).all():
            status_counts[status] = count

        contradiction_count = self.session.query(func.count(Contradiction.id)).join(Trace).filter(Trace.created_at >= cutoff).scalar() or 0

        model_counts = defaultdict(int)
        for model, count in self.session.query(Trace.model, func.count(Trace.id)).filter(Trace.created_at >= cutoff).group_by(Trace.model).all():
            model_counts[model] = count

        avg_confidence = self.session.query(func.avg(Claim.confidence)).join(Trace).filter(Trace.created_at >= cutoff).scalar() or 0.0
    ...
    def get_per_model_metrics(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        results = []
        for model in self.session.query(Trace.model).filter(Trace.created_at >= cutoff).distinct().all():
            model_name = model[0]
            model_traces = self.session.query(Trace).filter(
                and_(Trace.model == model_name, Trace.created_at >= cutoff)
            ).all()
            ...
            contradictions = self.session.query(Contradiction).filter(Contradiction.trace_id.in_(trace_ids)).all()
    ...
    def get_per_task_metrics(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        task_stats: dict[str, dict] = defaultdict(lambda: {"requests": 0, "claims": 0, "supported": 0, "unsupported": 0, "unclear": 0})
    ...
    def get_drift_metrics(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        daily_stats = defaultdict(lambda: {"requests": 0, "claims": 0, "supported": 0, "avg_confidence": 0.0})
    ...
    def get_recent_contradictions(self, limit: int = 50) -> list[dict[str, Any]]:
        contradictions = self.session.query(Contradiction).join(Trace).order_by(Trace.created_at.desc()).limit(limit).all()
